...
public class CustomerAction extends ActionSupport implements ServletRequestAware {
	...
	public String importExcel() throws IOException, InvalidFormatException {
		String filePath = servletRequest.getServletContext().getRealPath("/src/main/webapp/images");
		File fileToCreate = new File(filePath, this.fileUploadFileName);
		FileUtils.copyFile(this.fileUpload, fileToCreate);
		/*
		 * Workbook workbook = createWorkbook(new FileInputStream(fileUpload));
		 * 
	...
	public Workbook createWorkbook(InputStream is) throws IOException {
		if (fileUploadFileName.toLowerCase().endsWith("xls")) {
			return new HSSFWorkbook(is);
		}
		if (fileUploadFileName.toLowerCase().endsWith("xlsx")) {
